class CreditsWork 
{
	constructor(ip,port)
	{
		this.Connect = SignCS.Connect(`${ip}:${port}`);
	}
	
	Balance(key, callback)
	{
		key = this.CheckPublicKey(key);
        SignCS.Connect('http://165.22.220.8:8081').WalletBalanceGet(key, r => {
			if(r.status.code > 0)
			{
				throw r.status.message;
			}
			let b = [{balance:this.AmountToStr(r.balance)}];
			this.Connect.TokenBalancesGet(key, r => {
				if(r.status.code > 0)
				{
					throw r.status.message;
				}
