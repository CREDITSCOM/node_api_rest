...
let Net = (n,c) =>
{
    ...
    req.open("GET", `https://wallet.credits.com/Api/${n}/api/NetWork`, true);
	...
	req.onreadystatechange = function ()
	{
		if (req.readyState === 4)
		{
			...
			else
			{
				...
				{
					case "testnet":
						c([{ip:"dapps.credits.com",aport:"8081"}]);
					break;
					default:
						c([{ip:"main.credits.com",aport:"8081"}]);
				}
			}
			CloseLoader();
		}
...
function main()
{
	chrome.storage.local.get(['CS_PrivateKey','CS_PublicKey',"CS_NET"], function(d) {
		...
		else
		{
			...
			{
				Net("main",n => {
					n = {Name:"main", Url:n[0].ip, Port:n[0].aport,Mon:"CreditsNetwork"};
					chrome.storage.local.set({CS_NET: n});
					CS_NET = n;
					RenderMain();
				});
			}
...
{
	chrome.storage.local.get(["CS_Time"], d =>{
		let Tr = "";
		
		if(d.CS_Time != undefined)
		{
			for(let i in d.CS_Time)
			{
				let t = new Date(d.CS_Time[i]);
				Tr += `<tr>
					<td>${i}</td>
					<td>${t.getDate()}.${t.getMonth()}.${t.getFullYear()} ${t.getHours()}:${t.getMinutes()}</td>
		...
		$(".del").on("click",(event) => {
			chrome.storage.local.get(["CS_Time"], d =>{
				if(d.CS_Time != undefined)
				{
					d.CS_Time[$(event.target).parents("tr").children()[0].innerText] = undefined
					chrome.storage.local.set({CS_Time:d.CS_Time});
				}
				RenderApprovedSite();
			});
		});
...
function RenderMain()
{
				...
				<div class="top-line-part2">
					<div class="login">
						<select id="CS_Net" class="button-c button100">
							<option value="CreditsNetwork" Mon="CreditsNetwork">CreditsNetwork</option>
							<option value="testnet" Mon="testnet">TestNet</option>
						</select>
					</div>
					<div class="burgmenu">
	...
	RenderMenu();
	$(`#CS_Net [mon='${Mon}']`).prop('selected', true);
	$("#CS_Net").on("change",function(){
		Net($("#CS_Net :selected").val(),n => {
			Url = n[0].ip;
			Port = n[0].aport;
	...
	$("body").append(`
		<div class="loader-layer" id="loader-container">
			<div class="loader-wrap">
				...
				<button class="lcb">Close</button>
			</div>
		</div>
	`);
	$("#loader-container .lcb").on("click",CloseLoader);
...
function GenerateTrans()
{
	...
	try
	{
		RenderConfirmPayment({Trans: SignCS.CreateTransaction({
			source: CS_PublicKey,
			target: Target,
...
/*function DoubleToByts(p)
{
	let sign = p > 0 ? 0 : 1;
	s = Math.ceil(Math.log2(p)+ 0.5);
	q = p / Math.pow(2,s);
	if(q > 1)
	{
		s = s + 1;
	...
	m = Math.ceil(q * Math.pow(2,52) 0,5);
	return m;
}*/